...
REPO_DIR="$HOME/351/cam"
README="$REPO_DIR/README.md"
BRANCH="readme-snapshot-$(date -u +%Y%m%dT%H%M%SZ)"
REMOTE="origin"
...
# create branch, stage README, commit and push
git fetch "$REMOTE" || true
git checkout -b "$BRANCH"
git add "$README"
git commit -m "docs: add comprehensive README snapshot $(date -u +%Y-%m-%dT%H:%M:%SZ)"
git push -u "$REMOTE" "$BRANCH"

echo "README created, committed on branch $BRANCH and pushed to $REMOTE"
